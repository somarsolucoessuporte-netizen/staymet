...
export default async function GestorDashboardPage({
  ...
  try {
    ...
    console.log('[DB-TEST] Conexão OK:', dbTest)
    ...
    return (
      <div style={{ padding: 40, fontFamily: 'monospace', background: '#1a1a1a', color: '#ff4444', minHeight: '100vh' }}>
        <h2 style={{ color: '#fff', marginBottom: 16 }}>ERRO REAL:</h2>
        <pre style={{ whiteSpace: 'pre-wrap', fontSize: 13, marginBottom: 24 }}>
          {error?.message}
        </pre>
        <pre style={{ whiteSpace: 'pre-wrap', fontSize: 10, color: '#888', marginBottom: 32 }}>
          {error?.stack}
        </pre>
        <h3 style={{ color: '#fff', marginBottom: 12 }}>ENV CHECK:</h3>
        <pre style={{ fontSize: 12, color: '#0f0', lineHeight: 2 }}>
          DATABASE_URL: {process.env.DATABASE_URL
            ? 'DEFINIDA (' + process.env.DATABASE_URL.substring(0, 40) + '...)'
            : 'NAO DEFINIDA'}
          {'\n'}
          DIRECT_URL: {process.env.DIRECT_URL
            ? 'DEFINIDA (' + process.env.DIRECT_URL.substring(0, 40) + '...)'
            : 'NAO DEFINIDA'}
          {'\n'}
